...
public:
    ServerCS(){
        ...
        portNum = 22410;
        cout << "The ServerCS is up and running using UDP on port " << portNum << "\n";
            ...
            if(type == CLIENT_QUERY_UNI_REQUEST) {
                ...
                if (queryUniCourse(&messageRequest, courseInfo) == 1) {
                    ...
                    if (sendto(serverUdpSocket->getSocketID(), (char *) &result, sizeof(result), 0,
                               (struct sockaddr *) &clientAddr, clientAddrLen) == -1) {
                        perror("ServerCS failed to send message to UDP socket.\n");
                        continue;
                    }
...
int main() {
    ServerCS serverCS;
    thread threadServerCS(serverCS);
    threadServerCS.join();
}
